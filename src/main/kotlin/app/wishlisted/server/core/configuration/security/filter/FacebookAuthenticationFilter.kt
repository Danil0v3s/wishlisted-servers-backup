...
import app.wishlisted.server.core.configuration.oauth.FacebookProvider
...
class FacebookAuthenticationFilter(
    ...
    AntPathRequestMatcher(configProperties.facebookLoginUrl)
) {
    ...
    override fun attemptAuthentication(request: HttpServletRequest, response: HttpServletResponse): Authentication? {
        ...
        if (accessToken != null && !accessToken.isEmpty()) {
            val provider = FacebookProvider(accessToken)
    ...
    private fun getAuthentication(user: User?): Authentication? {
        if (user != null) {
            var managedUser = userService.findByEmail(user.email)
            if (managedUser == null) {
